...
void knapsack (vector<bool>& sol, int idx, int cw, int cv, int srv) {
  ...
  if (idx == n) {
    ...
    sol[idx] = false;
    knapsack(sol,idx+1,cw,cv,srv - v[idx]);
    
    sol[idx] = true;
    knapsack(sol,idx+1,cw + w[idx], cv + v[idx], srv - v[idx]);
  } 
}
...
void knapsack ( ) {
  ...
  int srv = 0; // sum of reamining values
  for (int x : v) srv += x;
  knapsack(sol,idx,cw,cv,srv);
}
